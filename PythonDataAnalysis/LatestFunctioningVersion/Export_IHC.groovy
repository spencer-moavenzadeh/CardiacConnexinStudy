...
for (entry in projectEntries) {
    try {
        ...
        QP.setImageData(imageData)
        
        def annotationFile = new File(outputDir, baseName + "-Annotation.csv")
        def annotations = getAnnotationObjects()
        
        if (annotations.size() > 0) {
            saveAnnotations(annotationFile, ANNOTATION_COLUMNS)
        ...
        if (detections.size() > 0) {
            saveDetections(detectionFile, DETECTION_COLUMNS)
...
def saveAnnotations(file, columns) {
    def annotations = getAnnotationObjects()
        ...
        for (annotation in annotations) {
            def values = []
             for (column in columns) {
                 def value = getObjectValue(annotation, column)
...
def saveDetections(file, columns) { 
    def detections = getDetectionObjects()
    ...
    file.withWriter('UTF-8') { writer ->
        ...
        for (detection in detections) {
            def values = []
             for (column in columns) {
                 def value = getObjectValue(detection, column)
                 values.add(value != null ? value.toString(): '')
             }
             writer.writeLine(values.join(','))
        }
...
def getObjectValue(pathObject, columnName) { 
    try {
        if (columnName == 'Image') {
            return getCurrentImageData().getServer().getMetadata().getName()
            ...
            return pathObject.getROI()?.toString() ?: ''
            ...
            return pathObject.getROI()?.getPerimeter() ? String.format("%.3f", pathObject.getROI().getPerimeter()) : ''
